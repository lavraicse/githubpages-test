...
We utilize NVIDIA [NeMo RL](https://github.com/NVIDIA-NeMo/RL) as the primary training framework, leveraging a Megatron backend for distributed scaling. We implement the <span>$\tau^2$</span>-bench sandbox environment inside NVIDIA [Nemo Gym](https://github.com/NVIDIA-NeMo/Gym), which provides a seamless integration with the Nemo RL library.
